...
STORE_BG = {
    ...
    "4": {"url": "https://i.ibb.co/PZNCzQS8/9d1247f8c186708963115d9ba0bc8977.jpg", "price": 6000, "name": "الأزرق الملكي", "color": "#00d4ff"},
...
async def generate_card(name, level, xp, avatar_url, bg_url, text_color):
    try:
        ...
        av = Image.open(BytesIO(av_res.content)).convert("RGBA").resize((190, 190))
        
        # قص الأفاتار دائري
        mask = Image.new("L", (190, 190), 0)
        ImageDraw.Draw(mask).ellipse((0, 0, 190, 190), fill=255)
        img.paste(av, (40, 55), mask)
        
        draw = ImageDraw.Draw(img)
        # ملاحظة: الخطوط الافتراضية بسيطة، يفضل رفع ملف default.ttf لتكبير الخط
        draw.text((270, 70), f"NAME: {name}", fill=text_color)
        draw.text((270, 140), f"LEVEL: {level}", fill=text_color)
        draw.text((270, 210), f"TOTAL XP: {format_num(xp)}", fill=text_color)
...
async def rank(it: discord.Interaction, member: discord.Member = None):
    ...
    card = await generate_card(target.display_name, u['level'], u['xp'], target.display_avatar.url, u.get('bg', DEFAULT_BG), u.get('t_color', 'white'))
    
    if card: await it.followup.send(file=discord.File(card, "rank.png"))
    else: await it.followup.send("❌ حدث خطأ في معالجة الصورة")

@bot.tree.command(name="store", description="متجر خلفيات الرتب")
async def store(it: discord.Interaction):
    emb = discord.Embed(title="🛒 متجر خلفيات Sky Data", description="شراء الخلفية يغير شكل بطاقة `/rank` الخاصة بك", color=0x9b59b6)
...
@bot.tree.command(name="buy", description="شراء خلفية من المتجر")
async def buy(it: discord.Interaction, number: str):
    ...
    await it.response.send_message(f"✅ اشتريت خلفية **{item['name']}**! جرب `/rank` الآن.")

@bot.tree.command(name="top", description="قائمة المتصدرين")
async def top(it: discord.Interaction):
    ...
    for i, u in enumerate(users, 1):
        ...
        name = m.mention if m else "مستخدم غادر"
        desc += f"**#{i}** | {name} - لفل `{u['level']}` - `{format_num(u['xp'])}` XP\n"
    emb.description = desc
    await it.followup.send(embed=emb)

# --- 6. أوامر الإدارة ---
@bot.tree.command(name="give_xp", description="إضافة نقاط لمستخدم")
async def give_xp(it: discord.Interaction, member: discord.Member, amount: int):
    ...
    await it.response.send_message(f"✅ تم منح {member.mention} `{amount}` نقطة.")
...
async def reset_all(it: discord.Interaction):
    ...
    await it.response.send_message("⚠️ تم مسح جميع البيانات!")

# --- 7. نظام التفاعل والترقيات ---
@bot.event
async def on_message(msg):
    ...
    if new_lvl > u['level']:
        ...
        if ch: await ch.send(f"🎊 {msg.author.mention} مبروك لفل **{new_lvl}**!")
...
app = Flask('')
@app.route('/')
def home(): return "Sky System Active"
